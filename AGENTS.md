...
#### 🏛️ الفصل الثالث عشر: دستور مساحات العرض اليمنى الموحدة (Unified Right-Side Workspace Standards)
لتأمين لغة تواصل بصري موحدة وقوية في كافة الأقسام اليمنى (Right-Side Workspaces) وتوحيد البطاقات المعروضة فيها، يجب تطبيق القواعد التالية مع مرونة التطوير المستمر:

1. **الحاوية الرئيسية الخارجية (Outer Shield Container):**
   * تعيش مساحات العمل اليمنى بالكامل داخل غلاف زجاجي كريستالي معتم ومغلق يحمي البيانات من التشتت البصري:
     `bg-[#0a0b10]/60 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl overflow-hidden relative`

2. **شريط التحكم والترويسة الكريستالي (The Command Header):**
   * **الأيقونة والتعريف:** يحتوي الركن الأيمن/الأيسر دائماً على مربع أيقونة زجاجي مطفأ ومحايد:
     `w-12 h-12 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white shrink-0`
   * **أزرار التبويب وتغيير العرض (View Switcher Pills):** التبديل بين الجدول والبطاقات يعتمد أزرار كبسولية زجاجية غاية في البساطة تعتمد التباين الفضي/الأبيض الناصع عند التفعيل.

3. **فيزياء البطاقات اليمنى المتنفسة (Right-Side Breathing Card Physics):**
   * **حالة السكون (Resting State):** خلفية صلبة داكنة جداً معتمة تمنح ثقلاً حقيقياً للبطاقة `bg-[#0a0a0f]`، مع مقروئية عالية جداً لكافة البيانات بفضل نصوص رمادية ناصعة ومقروءة بوضوح `text-slate-300` تمنع تعتيم البطاقة الكامل.
   * **حالة الاختيار والتنفس الفخم (Active Selection):**
     * **القفزة والظل:** تقفز البطاقة برفق إلى الأمام بتأثير تباين حجمي دافئ (`scale-[1.03]`) ممتد على زمن `duration-500` مع تظليل خفيف ملوّن بلون المحرك.
     * **الوهج المحيطي السفلي (Ambient Bottom Ray):** تفعيل وهج خلفي دائري عريض وخافت جداً خلف الجزء السفلي للبطاقة المحددة ينبثق برفق بنصف قطر مهدئ يمنح عمقاً بصرياً رائعاً (`bg-[theme]-500/25 rounded-full blur-xl absolute -bottom-10 left-1/2 -translate-x-1/2 w-28 h-16 pointer-events-none`).
     * **الأيقونة والإطار:** تضيء الأيقونة الداخلية بلونها النقي المشرق فقط دون تأثيرات النبض المشوش، مع إبقاء إطارها غلافاً زجاجياً مطفأً وهادئاً (`bg-white/5 border-white/10`).
     * **سماكة الحدود:** تزداد سماكة حدود البطاقة النشطة إلى خط زجاجي صلب وواضح (`border-2 border-[theme]-500`).
