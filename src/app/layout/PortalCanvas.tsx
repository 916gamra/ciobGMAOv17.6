...
export function PortalCanvas({ componentMap, user, onLogout }: { componentMap: Record<string, React.FC<any>>, user: User | null, onLogout: () => void }) {
  ...
  return (
    <main className="flex-1 flex flex-col min-w-0 relative z-10 bg-transparent overflow-hidden my-1 mr-3 ml-1">
      {/* Canvas Area with Glass Frame Effect */}
      <div className="flex-1 relative overflow-hidden flex flex-col w-full rounded-3xl bg-black/30 backdrop-blur-2xl border border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.6)]">
        <AnimatePresence mode="wait">
          {currentTab ? (() => {
            ...
            return (
              ...
              >
                <div className="flex flex-col p-4 md:p-6 lg:p-8 min-h-0 h-full">
                  <ErrorBoundary key={currentTab.id} portalId={currentTab.portalId} componentName={currentTab.title}>
