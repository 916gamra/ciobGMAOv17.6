...
export function PortalTabs() {
  ...
  // Card starts at LEFT_PADDING (8px), width = PRIMARY_WIDTH (215px), slant width = PRIMARY_SLANT_W (34px)
  const LEFT_PADDING = 8;
  const cardTopSlantX = LEFT_PADDING + (SVG_PATHS.PRIMARY_WIDTH - SVG_PATHS.PRIMARY_SLANT_W); // 8 + (215 - 34) = 189px
  
  // Generous, soothing clearance between the card's outer slant and the container's slant wall (increased for visual comfort)
  const COMFORT_SLANT_SPACING = 28; 
  
  // Outer slant parameters (clean 45° angle descending 46px down to shelf line):
  const slantStart = cardTopSlantX + COMFORT_SLANT_SPACING; // 189 + 28 = 217px
  const slantEnd = slantStart + 46; // 263px
  ...
  return (
    <PortalHeaderChassis
      ...
      slantEnd={slantEnd}
    >
      <div className="flex items-center h-[54px] pl-2 pr-2 z-10 shrink-0">
        ...
        {/* 2. Secondary Slanted Floating Tabs (Centered in the shelf space with isometric alignment) */}
        <div className="flex items-center h-full gap-0 ml-8">
