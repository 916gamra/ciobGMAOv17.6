...
import { FileText, Code2, BookOpen, Shield, Cpu, Terminal, Copy, Check, Search, Layers } from 'lucide-react';
...
export function DocumentationHubView() {
  ...
  return (
    <motion.div 
      ...
      className="flex flex-col h-full w-full space-y-6 overflow-y-auto p-4 md:p-6 custom-scrollbar text-white"
    >
      {/* Page Header */}
      <PageHeader
        title="مركز التوثيق والكتالوج الفني (Documentation Portal)"
        subtitle="دليل التشغيل الشامل، مواصفات OpenAPI Swagger، المعمارية الهندسية، وبروتوكولات الأمان."
        icon={<BookOpen className="w-6 h-6 text-indigo-400" />}
        badgeText="v17.1.0 Docs Hub"
        badgeColor="indigo"
        actions={
          <div className="flex items-center gap-1.5 bg-[#0a0a0f]/40 p-1.5 rounded-2xl border border-white/10">
            <button
            ...
            >
              <Code2 className="w-3.5 h-3.5" /> OpenAPI
            </button>
            <button
            ...
            >
              <FileText className="w-3.5 h-3.5" /> دليل المستخدم
            </button>
            <button
            ...
            >
              <Cpu className="w-3.5 h-3.5" /> دليل المطورين
            </button>
            <button
            ...
            >
              <Shield className="w-3.5 h-3.5" /> الأمان والأنماط
            </button>
          </div>
        }
      />
          ...
          <GlassCard className="p-5 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="bg-[#0a0a0f]/50 border border-white/10 rounded-xl pr-9 pl-3 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 w-64"
                />
              </div>
            </div>
            ...
            <div className="space-y-3 mt-4">
              {filteredPaths.map(([path, methods]: [string, any]) => (
                <div key={path} className="border border-white/10 rounded-2xl bg-[#0a0a0f]/40 overflow-hidden">
      ...
      {activeTab === 'user_guide' && (
        <GlassCard className="p-6 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6 text-sm">
            <div className="bg-[#0a0a0f]/40 border border-white/10 rounded-2xl p-5 space-y-3">
      ...
      {activeTab === 'dev_guide' && (
        <GlassCard className="p-6 space-y-6">
          ...
          <div className="bg-[#0a0a0f]/50 border border-white/10 rounded-2xl p-4 font-mono text-xs text-slate-300 overflow-x-auto">
      ...
      {activeTab === 'specs' && (
        <GlassCard className="p-6 space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-[#0a0a0f]/40 border border-white/10 rounded-2xl p-4 space-y-2">
