...
import { Save, FolderPlus, Layers, Hash, ArrowRight } from 'lucide-react';
...
export function PdrModals({ activeModal, onClose, families, templates, blueprints = [], user, onLinkTemplate }: PdrModalsProps) {
  ...
  const getModalProps = () => {
    switch (activeModal) {
      case 'family':
        return { title: 'New Family', icon: <FolderPlus className="w-5 h-5 text-cyan-400" /> };
      case 'template':
        return { title: 'New Template', icon: <Layers className="w-5 h-5 text-cyan-400" /> };
      case 'blueprint':
        return { title: 'New Blueprint', icon: <Hash className="w-5 h-5 text-cyan-400" /> };
      default:
                ...
                <div className="space-y-4 pt-2">
                  <div>
                    ...
                    <input
                      ...
                      disabled
                      value={activeSlotId}
                      className="titan-input font-mono opacity-80 bg-[#0a0a0f]/50 border-white/10 cursor-not-allowed text-cyan-400 text-lg text-center tracking-widest font-extrabold uppercase"
                    />
                  </div>
