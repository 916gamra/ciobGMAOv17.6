...
export function AnalyticsDashboardPage() {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="border border-white/10 p-4 rounded-xl shadow-2xl backdrop-blur-xl bg-slate-950/90 dir-ltr" dir="ltr">
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-[#0a0a0f] rounded-3xl border border-slate-200 dark:border-white/5 shadow-2xl text-slate-800 dark:text-slate-200 font-sans pb-4 overflow-hidden overflow-y-auto custom-scrollbar dir-ltr" dir="ltr">
      ...
      <div className="p-6 md:p-8 pb-0 shrink-0">
        <PageHeader
          title={t('analytics.title', 'Analytics & Indicators Hub')}
          subtitle={t('analytics.subtitle', 'Live tracking of factory performance, spare parts consumption, and technical asset health.')}
          icon={<Eye className="w-7 h-7 text-fuchsia-400" />}
          badgeText={t('analytics.badge', 'Performance Analytics')}
          badgeColor="fuchsia"
          className="mb-2"
        >
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <HeaderBentoCard
              title={t('analytics.totalStockVolume', 'Total Stock Volume')}
              subtitle="GLOBAL STOCK VOLUME"
              value={kpis.totalStockVolume}
              valueUnit={t('analytics.unit.part', 'pcs')}
              icon={<Database className="w-3.5 h-3.5" />}
              color="emerald"
            />
            <HeaderBentoCard
              title={t('analytics.distinctBlueprints', 'Distinct Parts')}
              subtitle="DISTINCT BLUEPRINTS"
              value={kpis.distinctParts}
              valueUnit={t('analytics.unit.type', 'types')}
              icon={<PackageSearch className="w-3.5 h-3.5" />}
              color="cyan"
            />
            <HeaderBentoCard
              title={t('analytics.monthlyRequisitions', 'Monthly Requisitions')}
              subtitle="MONTHLY REQUISITIONS"
              value={kpis.totalReqsThisMonth}
              valueUnit={t('analytics.unit.request', 'reqs')}
              icon={<TrendingUp className="w-3.5 h-3.5" />}
              color="purple"
            />
            <HeaderBentoCard
              title={t('analytics.stockHealthIndex', 'Stock Health Index')}
      ...
      <div className="flex flex-col flex-1 px-6 md:px-8 mt-6 gap-6 min-h-0">
            ...
            <GlassCard className="h-full flex flex-col relative overflow-hidden group !p-8 shadow-[0_0_30px_rgba(0,0,0,0.5)] border-white/5 ">
              ...
              <div className="relative z-10 mb-8">
                <h2 className="text-xl font-bold text-white mb-2 uppercase tracking-tight flex items-center gap-3">
                  ...
                  {t('analytics.topConsumingMachines', 'Top Consuming Machines / الآلات الأكثر استهلاكاً')}
                </h2>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mr-11">{t('analytics.topConsumingMachinesDesc', 'Production equipment and assemblies with highest material withdrawals / المعدات الأكثر سحباً للوحدات المادية')}</p>
              </div>
              <div className="flex-1 relative z-10 -ml-4 min-h-0">
                {topMachines.length > 0 ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-500">
                    ...
                    <p className="text-[10px] uppercase font-bold tracking-widest">{t('analytics.insufficientData', 'Insufficient storage withdrawal data / بيانات سحب المخزن غير كافية')}</p>
                  </div>
                )}
              </div>
            </GlassCard>
            ...
            <GlassCard className="h-full flex flex-col relative overflow-hidden group !p-8 shadow-[0_0_30px_rgba(0,0,0,0.5)] border-white/5 ">
              ...
              <div className="relative z-10 mb-8">
                <h2 className="text-xl font-bold text-white mb-2 uppercase tracking-tight flex items-center gap-3">
                  ...
                  {t('analytics.stockHealthIndicator', 'Stock Health Indicator / مؤشر صحة المخزن')}
                </h2>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mr-11">{t('analytics.stockHealthIndicatorDesc', 'Classification of spare parts availability and shortages / تصنيف مستويات النواقص والوفرة لقطع الغيار')}</p>
              </div>
              <div className="flex-1 relative z-10 flex flex-col items-center min-h-0">
                ...
                <div className="flex flex-wrap justify-center gap-x-6 gap-y-3 mt-4 shrink-0">
                  {stockHealth.map(item => {
                    ...
                    if (item.name === 'Healthy') statusLabel = t('analytics.health.healthy', 'Healthy Stock / رصيد آمن ومستقر');
                    if (item.name === 'Low Stock') statusLabel = t('analytics.health.low', 'Low Stock / تحت الحد الأدنى');
                    if (item.name === 'Out of Stock') statusLabel = t('analytics.health.out', 'Out of Stock / رصيد منتهي تماماً');
                    return (
                      <div key={item.name} className="flex items-center gap-2 bg-white/[0.02] border border-white/5 px-3 py-1.5 rounded-lg">
                        ...
                        <span className="text-[10px] uppercase font-bold tracking-widest text-slate-400">{statusLabel} <span className="text-white ml-2">{item.value}%</span></span>
                      </div>
                    );
                  })}
                </div>
        ...
        {/* Bottom Row */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5 }} className="shrink-0 mb-6">
            ...
            <div className="relative z-10">
              <div className="mb-8">
                <h2 className="text-xl font-bold text-white mb-2 uppercase tracking-tight flex items-center gap-3">
                  ...
                  {t('analytics.topTechnicians', 'Top Requisitioning Technicians / الفنيون الأكثر نشاطاً')}
                </h2>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mr-11">{t('analytics.topTechniciansDesc', 'Activity rates monitoring of technical spare parts withdrawals / مراقبة حركات صرف المستندات الفنية')}</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-5">
                {techActivity.length > 0 ? techActivity.map((tech, idx) => (
                  <div key={idx} className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 flex flex-col items-center justify-center text-center hover:border-fuchsia-500/30 hover:bg-fuchsia-500/5 transition-all group duration-300 relative overflow-hidden">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mt-2 bg-[#0a0a0f]/40 px-3 py-1 rounded-md border border-white/5 group-hover:text-fuchsia-400 transition-colors">{tech.count} {t('analytics.unit.withdrawal', 'withdrawals / عمليات سحب')}</span>
                  </div>
                )) : (
                  <div className="col-span-full py-12 flex flex-col items-center justify-center text-center border border-dashed border-white/10 rounded-3xl ">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">{t('analytics.waitingForWithdrawals', 'Waiting for withdrawals data / في انتظار تسجيل عمليات السحب لبناء الإحصائيات')}</p>
                  </div>
                )}
              </div>
            </div>
